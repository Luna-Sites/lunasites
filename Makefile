...
PROJECT_NAME=lunasites
STACK_NAME=luna-sites-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://lunasites.localhost"
...
acceptance-frontend-image-build:
	...
	@docker build frontend -t /Luna-Sites/lunasites-frontend:acceptance -f frontend/Dockerfile --build-arg VOLTO_VERSION=$(VOLTO_VERSION)

.PHONY: acceptance-backend-image-build
acceptance-backend-image-build:
	@echo "Build acceptance backend image"
	@docker build backend -t /Luna-Sites/lunasites-backend:acceptance -f backend/Dockerfile.acceptance --build-arg PLONE_VERSION=$(PLONE_VERSION)
...
acceptance-frontend-container-start:
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name lunasites-frontend-acceptance --link lunasites-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d /Luna-Sites/lunasites-frontend:acceptance

.PHONY: acceptance-backend-container-start
acceptance-backend-container-start:
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name lunasites-backend-acceptance -d /Luna-Sites/lunasites-backend:acceptance
...
acceptance-containers-stop: ## Stop Acceptance containers
	...
	@docker stop lunasites-frontend-acceptance
	@docker stop lunasites-backend-acceptance
