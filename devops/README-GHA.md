...
2. Go to your repository at [/Luna-Sites/lunasites](https://github.com/luna-sites/lunasites).
3. Click on `Settings` at the top-right corner.
4. In the left sidebar, click on `Environments`.
5. Press `New environment`.
6. Name the environment `luna-sites.com` and hit `Configure environment`.
...
|-------------|--------------|-------------|
| DEPLOY_HOST | luna-sites.com | The hostname or IP address of your Docker Swarm manager. |
...
- [Backend Tests Workflow](https://github.com/luna-sites/lunasites/actions/workflows/backend.yml)
- [Frontend Tests Workflow](https://github.com/luna-sites/lunasites/actions/workflows/frontend.yml)

Upon successful completion of the tests, Docker images for the Backend (`ghcr.io/luna-sites/lunasites-backend`) and Frontend (`ghcr.io/luna-sites/lunasites-frontend`) will be available.

### Initiating Manual Deployment

1. Navigate to [Manual Deployment of luna-sites.com](https://github.com/luna-sites/lunasites/actions/workflows/manual-deploy.yml).
...
- Initiate a new deployment using the `devops/stacks/luna-sites.com.yml` stack.
