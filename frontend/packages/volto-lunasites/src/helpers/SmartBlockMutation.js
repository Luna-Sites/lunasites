...
export const smartMutateBlock = (editor, blockId, newBlockData, formData) => {
  console.log('smartMutateBlock called with:', {
    blockId,
    newBlockData,
    formDataKeys: Object.keys(formData),
    formDataSample: {
      blocks: formData.blocks ? Object.keys(formData.blocks).slice(0, 3) : null,
      blocks_layout: formData.blocks_layout,
      hasCurrentBlock: formData.blocks ? !!formData.blocks[blockId] : false
    }
  });

  const { selection } = editor;
  if (!selection) {
    console.log('No selection, returning original formData');
  ...
  if (!match) {
    console.log('No matching block found, returning original formData');
    return { formData, newBlockId: null };
  }
  ...
  const endOfLine = nextNewlineIndex === -1 ? blockText.length : cursorOffset + nextNewlineIndex;
  ...
  const isOnlyWhitespace = (textBeforeSlashLine + textAfterSlashLine).trim() === '' && 
                          leadingSpaces.length === startOfLine && 
                          textAfterSlash.trim() === '';

  console.log('Text analysis:', {
    currentLine,
    slashMatch: slashMatch ? slashMatch.slice(0, 4) : null,
    textBeforeSlashLine,
    textAfterSlashLine,
    textAfterSlash,
    leadingSpaces,
  ...
  if (isOnlyWhitespace) {
    ...
    console.log('Case 1: Only whitespace, replacing entire block');
    ...
    console.log('Case 2/3: Has content, need to split intelligently');
    
    // If there's text before slash line, keep it in current block
    if (textBeforeSlashLine.trim()) {
      console.log('Has text before slash line, keeping in current block');
  ...
  console.log('Returning updated formData:', {
    originalFormDataKeys: Object.keys(formData),
    newFormDataKeys: Object.keys(newFormData),
    changed: formData !== newFormData,
    blocksCount: Object.keys(blocks).length,
    layoutItemsCount: blocksLayout.items.length,
    createdBlockId
  });
