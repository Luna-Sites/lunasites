...
const ColorTestDemo = () => {
  ...
  React.useEffect(() => {
    const updateColors = () => {
      ...
      setColors({
        background: style.getPropertyValue('--lunasites-background-color').trim(),
    ...
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '10px', marginBottom: '20px' }}>
        {Object.entries(colors).map(([name, color]) => (
          <Card key={name} style={{ backgroundColor: color, padding: '10px', minHeight: '80px' }}>
            <div style={{ 
              color: name === 'background' ? '#333' : 'white',
              textShadow: name === 'background' ? 'none' : '1px 1px 2px rgba(0,0,0,0.5)'
            }}>
      ...
      <div style={{ 
        ...
        <h3 style={{ color: 'var(--lunasites-header-color)', margin: '0 0 10px 0' }}>
          Live CSS Variables Test
        </h3>
        <p style={{ margin: '0 0 10px 0' }}>
          This section uses CSS variables directly and should update when you change colors.
        </p>
      ...
      <div style={{ fontSize: '12px', color: '#666' }}>
        <strong>Instructions:</strong> Go to any page edit mode, look for "Color Schema" in the sidebar, and change colors to see this demo update in real-time.
      </div>
    </div>
  );
};
