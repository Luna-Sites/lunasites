...
export const ContentsItemComponent = ({
  ...
  // Check if item has preview image, is an image, or is a video
  const hasImage = item.image || item['@type'] === 'Image' || item.preview_image;
  const hasVideo = isVideoFile(item);
  const imageUrl = item.image?.scales?.preview?.download || 
                   item.preview_image?.scales?.preview?.download ||
                   (item['@type'] === 'Image' ? item['@id'] + '/@@images/image/preview' : null);
  const videoUrl = hasVideo ? (item.file?.download || item['@id'] + '/@@download/file') : null;
